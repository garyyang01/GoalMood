...
- 當團隊成員有超過 10 個目標時會發生什麼？（顯示行為、捲動或 UI 限制）
- 當目標描述接近或達到 500 字元上限時如何顯示？（UI 截斷或換行處理）
- 當團隊成員姓名接近或達到 50 字元上限或包含特殊字元時如何顯示？（顯示格式）
- 當團隊成員從未設定心情時，系統如何顯示心情？（預設狀態指示器）
- 當所有團隊成員都已完成所有目標時會發生什麼？（視覺回饋或慶祝）
- 當系統中不存在團隊成員時會發生什麼？（空白狀態訊息）
- 當多個使用者同時編輯同一項資料時會發生什麼？（併發控制策略）

## 需求 *(必填)*

### 功能需求

- **FR-001**：系統必須在儀表板視圖中以個別卡片顯示所有團隊成員
- **FR-002**：系統必須在每個團隊成員的卡片中顯示其姓名、當前心情表情符號和目標清單
- **FR-003**：系統必須顯示每個團隊成員的目標完成計數（例如「已完成 2/3」）
- **FR-004**：系統必須提供新增目標的表單，包含團隊成員選擇下拉選單和目標描述文字輸入
- **FR-005**：系統必須提供更新團隊成員心情的表單，包含團隊成員選擇下拉選單和心情表情符號選擇器（😀 😊 😐 😞 😤）
- **FR-006**：系統必須允許使用者透過儀表板上的核取方塊標記目標為完成
- **FR-007**：系統必須允許使用者取消勾選已完成的目標以將其回復為未完成狀態
- **FR-008**：系統必須顯示統計面板，顯示團隊目標完成百分比
- **FR-009**：系統必須在統計面板中顯示心情分布計數（每種心情類型的計數）
- **FR-010**：系統必須持久化團隊成員、目標和心情，使資料在應用程式重啟後仍然存在
- **FR-011**：系統必須在目標被標記為完成/未完成時即時更新完成計數和統計資料
- **FR-012**：系統必須在心情變更時即時更新心情顯示
- **FR-013**：系統必須在新增目標時驗證已選擇團隊成員
- **FR-014**：系統必須在新增目標時驗證目標描述不為空且不超過 500 字元
- **FR-015**：系統必須在更新心情時驗證已選擇團隊成員
- **FR-016**：系統必須在更新心情時驗證已選擇心情表情符號
- **FR-017**：系統必須按新增順序顯示目標
- **FR-018**：系統必須預設將目標關聯到今天的日期（目標是每日的）
- **FR-019**：系統必須支援從團隊成員卡片刪除目標，並在刪除前顯示確認對話框
- **FR-020**：系統必須在不存在目標或團隊成員時顯示適當的空白狀態
- **FR-021**：系統必須保留所有歷史目標資料（包括前幾天的目標）但在儀表板上僅顯示今日目標
- **FR-022**：系統必須在目標實體中記錄建立日期，以區分今日目標與歷史目標
- **FR-023**：系統必須採用最後寫入勝出策略處理併發更新（不檢測或阻止衝突）
- **FR-024**：系統必須驗證團隊成員姓名不為空且不超過 50 字元

### 關鍵實體

- **TeamMember（團隊成員）**：代表團隊中的個人。關鍵屬性包括唯一識別碼、姓名（最大 50 字元）和當前心情狀態（五種表情符號選項之一：開心、滿足、中性、悲傷、壓力）。

- **Goal（目標）**：代表分配給團隊成員的每日任務或目標。關鍵屬性包括唯一識別碼、描述文字（最大 500 字元）、完成狀態（完成/未完成）、關聯的團隊成員和建立日期（預設為今天）。

- **Mood（心情）**：代表團隊成員在某個時間點的情緒狀態。以五種表情符號選項之一表示：😀（開心）、😊（滿足）、😐（中性）、😞（悲傷）、😤（壓力）。每個團隊成員有一個當前心情。

## 成功標準 *(必填)*

### 可衡量結果

- **SC-001**：團隊成員可以在開啟應用程式後 2 秒內檢視完整的團隊狀態（所有成員、目標和心情）
- **SC-002**：團隊成員可以在從存取表單到儀表板顯示的 10 秒內新增新目標
- **SC-003**：團隊成員可以在從存取表單到儀表板顯示的 5 秒內更新心情
- **SC-004**：團隊成員可以透過單次互動（一次點擊/輕觸）標記目標為完成
- **SC-005**：應用程式以數學準確性正確顯示完成百分比（例如 3 個目標中的 2 個 = 66.67%）
- **SC-006**：統計面板在任何目標或心情變更後 1 秒內更新
- **SC-007**：所有資料正確持久化 - 100% 的目標、心情和團隊成員在應用程式重啟後仍然存在
- **SC-008**：應用程式支援至少 10 個團隊成員而不會出現效能下降
- **SC-009**：應用程式支援所有團隊成員總共至少 50 個目標而不會出現效能下降
- **SC-010**：當發生無效提交時，表單驗證錯誤立即顯示（200 毫秒內）
